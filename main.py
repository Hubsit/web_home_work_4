    ...
    try:
        ...
        if BASE_DIR.joinpath('storage/data.json').exists():
            with open(BASE_DIR.joinpath('storage/data.json'), 'r', encoding='utf-8') as fd:
                data = json.load(fd)
            data[str(datetime.now())] = payload
        else:
            data = {str(datetime.now()): payload}
        with open(BASE_DIR.joinpath('storage/data.json'), 'w', encoding='utf-8') as fd:
            json.dump(data, fd, ensure_ascii=False)
    except ValueError as error:
        logging.error(f'Field parse data {body}: {error}')
    except OSError as error:
        logging.error(f'Field write data{body}: {error}')
...
if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO, format='%(threadName)s %(messages)s')
